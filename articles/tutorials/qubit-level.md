...
在我们的示例中，我们将定义一个 Q# 操作来执行 qubit 量程傅立叶转换，并将以下表示形式作为线路：
...
### <a name="namespaces-to-access-other-no-locq-operations"></a>用于访问其他操作的命名空间 Q#
...
`H`接下来，我们会看到，下一个入口是在 `R1` 第一个 qubit (上，并由第二个/第三个) 控制的入口：
...
因此，我们已完成将量程傅立叶转换的 qubit 级操作写入到操作中 Q# ：

<img src="../media/qft_full.PNG" alt="Three qubit quantum Fourier transform circuit diagram" width="600">

但是，我们现在不能调用它。
我们分配 qubits 时，我们的状态为 $ \ket {0} $，但在生活中， Q# 我们应该按照与 ) 相同的方式来保留它们 (或更好的！。
...
要求将所有释放的 qubits 显式设置为 $ \ket {0} $ 是的一项基本功能 Q# ，因为当它们开始使用同一 qubits (稀有资源) 时，它可以精确地了解其状态。
...
> 通常， Q# 操作对运行它们的计算机是不可知的，但某些功能（例如）的 `DumpMachine` 行为可能有所不同。

4. 执行模拟时，操作调用将返回在文件中定义的值 Q# 。
...
根据给定基础的度量值 (例如，计算基础 $ \{ \ket {0} ，\ket {1} \} $) ，将 qubit 状态投影到---的度量值，从而破坏两者之间的任何 superposition。

若要在程序内实现度量 Q# ，我们使用 `M`) 的操作 (`Microsoft.Quantum.Intrinsic` ，这将返回一个 `Result` 类型。
...
在 `using`) 语句之前， `Result` 为每个 qubit) 声明并绑定此 qubits 数组 (一个 (数组，然后再分配它： 
...
> [!NOTE]
> 此语句的语法对是唯一的 Q# ，但与 `resultArray[i] <- M(qs[i])` 其他语言（如 F # 和 R）中所示的类似变量重新分配相对应。
...
## <a name="use-the-no-locq-libraries"></a>使用 Q# 库
