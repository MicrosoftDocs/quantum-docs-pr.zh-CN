...
除了将每个 $N _e $ 电子分配到特定状态（或 orbital），第二个量化都将跟踪每个 orbital 并存储每个中是否存在 electron，同时自动确保对应的波形函数。
...
也就是说，有效的基本状态为以下形式： $ \ psi_ {0，\uparrow}，\ldots，\ psi_ {N-1，\uparrow}，\ psi_ {0，\downarrow}，\ldots，\ psi_ {N-1，\downarrow} $，其中 $ \uparrow $ 和 $ \downarrow $ 是指定任意.
...
\end{align} 请注意，此处 $a ^ \ dagger_1 \ket{1}_1 = 0 $ 和 $a _1 \ket{0}_1 $ yield 0 矢量 not $ \ket{0}_1 $。
...
\begin{equation} \hat{H} = \sum\_{pq} H\_{pq} a ^ \dagger\_p a\_q + \frac{1}{2}\sum\_{pqrs} H\_{pqrs} a ^ \dagger\_p a ^ \dagger\_q a\_ra\_s + H\_{\textrm nuc}，\label{eq： totalHam} \end{equation}，其中 $h\_{\textrm nuc} $ 是核能量（这是一种在近似值下的常量），

\begin{align} h\_{pq} & = \int\_{-\infty} ^ \infty \psi ^\*\_p （x\_1） \left （-\frac{\nabla ^ 2}{2} + V （x\_1） \right） \psi\_q （x\_1） \mathrm{d} ^ 3\_1，\end{align}

其中 $V （x） $ 是可能的平均字段，并且

\begin{align} h\_{pqrs} & = \int\_{-\infty} ^ \infty \int\_{-\infty} ^ \infty\psi\_p ^\*（x\_1） \psi\_q ^\*（x\_2） \left （\frac{1}{| x_1-x_2 |} \right） \psi\_r （x\_2） \psi\_s （x\_1） \mathrm{d} ^ 3\_1 \ mathrm {d} ^ 3\_2。 \ 标签 {eq：积分} \end{align}

\_{pq} $ $h 术语称为 electron 整型，因为所有这类条款仅涉及单个电子，同样 $h\_{pqrs} $ 是两个 electron 整型。
...
例如，在 $p、q、r、s $ 是不同的索引，我们可以使用反通信规则显示： $ $ a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} =-a ^ \dagger\_{q} a ^ \dagger\_{p} a\_{r} a\_{s} =-a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{s}\_{r} = a ^ \dagger\_{q} a ^ \dagger\_{p} a\_{r}\_{s}。
$$

此外，由于 $H $ 是 Hermitian 的，因此每个非 Hermitian fermionic 运算符都假设 $h\_{pqrs} a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} $，还可以在 $H $ 中找到 Hermitian 共轭。 为了唯一索引按这些 symmetries 的术语组，我们在索引 $ （i\_1、\cdots、i\_n、j\_1、\cdots、j\_m）上定义一个规范排序，其中任何 $n + m $ fermionic 运算符序列 $a ^ \dagger\_{i\_1} \cdots a ^ \dagger\_{i\_n}\_{j\_1} \cdots:\_\_
