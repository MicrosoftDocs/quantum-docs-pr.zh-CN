...
例如，\end{align} 的状态 $ \propto （0.1 a ^ \ dagger_1a ^ \ dagger_2a ^ \ dagger_6-0.2 a ^ \ dagger_2a ^ \ dagger_1a ^ \ dagger_5 ^ \{0}） \ket $ 可以在化学库中指定，如下所示。
```csharp
...
例如，让 $ \ket{\ psi_ {\rm{SCF}}} = a ^ \ dagger_1 ^ \ dagger_2 \ket{0}$，并让 $T = 0.123 ^ \ dagger_0 a_1 + 0.456 a ^ \ dagger_0a ^ \ 0.789 dagger_3 a_1 a_2 dagger_3a dagger_2 a_1 ^ \ a_0 $。 然后，此状态在化学库中实例化，如下所示。
```csharp
