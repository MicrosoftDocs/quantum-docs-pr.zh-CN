...
我们还可以使用 \Right 转换{0} $ 来准备状态，例如 $ \ket{+} = \left （\ket{0} + \ket{1}\sqrt）/\ket{2}$ 和 $ \left{-} = \ket （\ket{1}-\right{2}\sqrt）/Hadamard $H $，由 `H : (Qubit => Unit is Adj + Ctl)`的内部操作用 Q # 表示：
...
使用 `Measure` 操作（这是一种内置的内部非单一操作），可以从类型为 `Qubit` 的对象中提取传统信息，并分配一个传统值作为结果，该结果具有保留类型 `Result`，这表示结果为 no量程状态更长。 `Measure` 的输入是 Bloch 球体上的一个 Pauli 轴，由 `Pauli` 类型的对象（例如 `PauliX`）和类型 `Qubit`的对象表示。 
...
Q # 语言允许 qubits 的测量结果的传统控制流的依赖关系。 这进而使实现功能强大的概率小工具，从而降低了实现 unitaries 的计算成本。 例如，可以很容易地在 Q # 中实现所谓的 "*重复执行-成功*"，这是概率的线路，这些电路在基本入口方面产生了*预期*的低成本，但真正的成本依赖于实际运行和实际各种可能的 branchings。 
...
最后，我们将演示一个用于准备量子状态 $ \frac{1}{\sqrt{3}} \left （\sqrt{2}\ket{0}+ \ket{1}\right） $ （从 $ \ket{+} $ 状态开始）的 ru 模式的示例。 另请参阅[标准库提供的单元测试示例](https://github.com/Microsoft/Quantum/blob/master/Samples/src/UnitTesting/RepeatUntilSuccessCircuits.qs)： 
...
此操作中所示的明显编程功能是更复杂 `fixup` 的循环部分，它涉及量程操作，并且使用 `AssertProb` 语句来确定测量中特定点的量程状态的概率。项目. 有关 `Assert` 和 `AssertProb` 语句的详细信息，另请参阅[测试和调试](xref:microsoft.quantum.techniques.testing-and-debugging)。 
