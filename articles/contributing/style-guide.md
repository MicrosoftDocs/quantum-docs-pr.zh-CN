...
如果操作或函数不能直接使用，而是应由部分应用程序操作的匹配可调用的使用，请考虑使用以为部分应用的可调用的开头的名称 `_` 。
...
|   | 名称 | 说明 |
|---|------|-------------|
| ☒ | <s>`ApplyDecomposedOperation_`</s> | 下划线 `_` 不应出现在名称的末尾。 |
| ☑ | `_ApplyDecomposedOperation` | 开头的下划线 `_` 清楚地指示此操作仅供内部使用。 |
