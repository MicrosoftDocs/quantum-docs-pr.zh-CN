...
量程开发工具包通过 NuGet 包提供对量程化学应用程序的支持 [`Microsoft.Quantum.Chemistry`](https://www.nuget.org/packages/Microsoft.Quantum.Chemistry) 。
...
若要使用这些示例测试安装，请克隆[主示例存储库](https://github.com/Microsoft/Quantum)，然后运行其中一个示例。  例如，若要运行该 [`MolecularHydrogen`](https://github.com/microsoft/Quantum/tree/master/samples/chemistry/MolecularHydrogen) 示例：
...
具体而言， `Microsoft.Quantum.Chemistry` 包提供的工具可用于加载[Broombridge 架构](xref:microsoft.quantum.libraries.chemistry.schema.broombridge)中所示的量程化学模拟问题的实例，还支持在最新版本的 NWChem 中导出。
