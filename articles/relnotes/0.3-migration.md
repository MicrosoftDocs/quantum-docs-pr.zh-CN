...
我们非常高兴地介绍了量子开发工具包的最新更新。  此页概述了新的 Q # 语言功能，并介绍了如何将现有的 Q # 程序[迁移](#Migration)到新的语法。  请参阅[此处](xref:microsoft.quantum.guide.basics)完整的 Q # 语言说明文档。
...
条件表达式是形式的表达式 `condition ? caseTrue | caseFalse` 。
正如上面的示例所示，现在还支持对数组进行迭代。

和中的分配 `using` `borrowing` 新适用于单个 qubits、qubit 数组和其所在的嵌套元组：
```qsharp
    using (qubit = Qubit()) {
...
同时引入了两个新的特殊化生成器指令 `invert` ，以 `distribute` `auto` 提高控制生成函子专用化的方式。
在操作中显式声明默认 `body` 专用化不再是必需的。
...
修饰符的绑定比任何其他表达式连结符，特别是比调用表达式连结符更严格 `(` `)` 。 
`Adjoint`、 `Controlled` 和新的 "解包" 后缀运算符 `!` 被视为表达式修饰符。 `Adjoint`将和 `Controlled` 视为表达式修饰符消除了许多情况下需要使用括号。 例如，下面的表达式是有效的后续 `Op` 操作、单一操作、 `opArr` 单一操作的数组和 `arg` 合适的参数： 
```qsharp
    Adjoint Op (arg);
    Controlled opArr[i] (arg);
```
迁移脚本将删除大多数不必要的括号，并充分利用新功能。 但请注意，脚本不会解释代码， `Controlled` 在极少数情况下将不考虑函子的不同解释（仅与部分应用程序结合相关）。

新修饰符将 `!` 用户定义类型转换为其基础类型。 此强制转换已被视为以前版本中的转换，并且已自动执行。 对于用户定义类型的新解释，这不再是这样的，需要显式转换。    
下面的示例演示如何使用 new 修饰符： 
```qsharp
...
```
从示例中可以看出， `!` 绑定比前缀修饰符更严格，如 `Controlled` 和 `Adjoint` 。

## <a name="breaking-changes"></a><a name="BreakingChanges"></a>重大更改 
...
除了下面列出的更改之外，模式 `__*__` （其中 * 是任何非空白字符序列）都是为供内部使用而保留的，不能用作符号名称。  
...
此类更改的一个示例是用户定义的专用化中的符号元组。 在以前的版本中，在以前的版本中，受控制或受控的 adjoint 专用化声明指定了单个符号参数，从0.3 开始，建议为所有特殊化提供此类符号元组，以 `...` 指示编译器应在需要时迁移可调用声明中的符号。 

## <a name="semantic-changes"></a>语义更改 

0.3 版本的最大有影响力是将用户定义类型集成到类型系统中的方式。 在以前的版本中，用户定义类型被认为是其基础类型的子类型。 在此版本中，将不再出现这种情况。 用户定义的类型现在被视为自己的不同类型，并且不存在用户定义类型与其基础类型之间的自动强制转换。 所有强制转换都需要通过 "解包" 运算符进行显式转换 `!` ，并且需要强制转换才能访问用户定义类型的对象的内容。  
...
下面的示例演示如何使用函数将 `ToOperation` 函数更改 `Square` 为操作 `op` ： 
```qsharp
...
## <a name="migrating-existing-projects-to-q-03"></a><a name="Migration"></a>将现有项目迁移到 Q # 0。3 
...
2. 在文本编辑器中，编辑 .csproj 文件，将所有 "0.3.1811.2802" 的版本更改为 `PackageReference` 版本-preview，例如：
```xml
...
4. 在命令行中，使用集成到命令行编译器中的格式设置工具通过运行以下命令来解决所有语法更改：`dotnet msbuild /t:qsharpformat`  
    - 文件将就地迁移。 将所有原始文件的备份复制到`obj\qsharp\.backup`
...
## <a name="migration-script"></a><a name="MigrationScript"></a>迁移脚本 
...
- 该项目是利用以前版本的量程开发工具包生成的，以确保迁移可能会成功。 可以通过对脚本使用参数来跳过此步骤 `-Force` 。
- 已安装新项目模板。
...
> 若要允许脚本 `qdk-migrate.ps1` 在 Windows 上运行，你可能需要更改执行策略。
